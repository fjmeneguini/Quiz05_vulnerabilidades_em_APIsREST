...
1. Instale as dependências na raiz do projeto com `npm install`.
2. Instale as dependências em cada microsserviço com `npm install` dentro de `auth-service`, `product-service` e `gateway`.
3. Verifique se o MySQL está disponível e se os bancos `auth_db` e `product_db` existem.
4. Inicie os serviços com `npm start` na raiz do projeto.